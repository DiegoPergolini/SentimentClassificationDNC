...
def getBalancedReviewsAndOverall(numPositiveTraining,numNegativeTraining,numPositiveTesting,numNegativeTesting,fileName,max_lenght,randomized = False,seed = 9):
    ...
    while posT < numPositiveTraining or negT < numNegativeTraining:
        raw = in_file.readline()

        jsonLine = json.loads(raw)
        if len(jsonLine['reviewText']) >0:
            review = jsonLine['reviewText']
            num_words = len(nltk.word_tokenize(review))
            if num_words<max_lenght and num_words > 2:
                if randomized:
                    if random.Random(seed).randint(0, 1) == 1:
                        if jsonLine['overall'] < 3.0:
                            if negT < numNegativeTraining:
                                current = (jsonLine['reviewText'], jsonLine['overall'])
    ...
    ranT.shuffle(listT)
    while posTe < numPositiveTesting or negTe < numNegativeTesting:
        raw = in_file.readline()
        ...
        if len(jsonLine['reviewText']) >0:
            ...
            if num_words<max_lenght and num_words > 2:
                if randomized:
                    if random.Random(seed).randint(0, 1) == 1:
                        if jsonLine['overall'] < 3.0:
                            if negTe < numNegativeTesting:
                                current = (jsonLine['reviewText'], jsonLine['overall'])
                                listTe.append(current)
                                negTe += 1
                        else:
                            if jsonLine['overall']> 3.0:
                                if posTe < numPositiveTesting:
                                    current = (jsonLine['reviewText'], jsonLine['overall'])
                                    listTe.append(current)
                                    posTe += 1
                else:
...
def getCrossDomainReviewsAndOverall(numTraining,numTesting,fileTraining,fileTesting,max_lenght,randomized= Falsem,seed=9):
...
def getW2vRepresentation(text,loadedModel):
    countDeleted = 0
    toReturn = []
    for element in nltk.word_tokenize(text):
        try:
